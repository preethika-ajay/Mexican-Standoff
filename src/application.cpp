...
public:
    ...
    void addMuzzleFlash(const glm::vec3& position) {
            ...
            p.color = glm::vec3(1.0f, glm::linearRand(0.5f, 1.0f), 0.0f);            
    ...
    void addBulletTrail(const glm::vec3& position) {
        ...
        p.color = glm::vec3(0.8f, 0.8f, 0.9f);         
        p.lifetime = 0.2f;
        p.maxLifetime = p.lifetime;
        p.size = 0.08f;
        particles.push_back(p);
    ...
    void addHitEffect(const glm::vec3& position) {       
        for (int i = 0; i < 30; ++i) {
            ...
            p.color = glm::vec3(1.0f, glm::linearRand(0.0f, 0.3f), 0.0f); 
...
void buildWindmill() {   
    root = std::make_unique<WindmillNode>(
        ...
        WesternPrimitive(WesternPrimitive::CUBE,
            ...
            /*color*/ glm::vec3(0.45f, 0.30f, 0.18f))   
    );
    ...
    auto towerMid = std::make_unique<WindmillNode>(
        ...
        WesternPrimitive(WesternPrimitive::CYLINDER,
            ...
            /*color*/ glm::vec3(0.62f, 0.43f, 0.24f))   
    );
    ...
    auto towerTop = std::make_unique<WindmillNode>(
        "TowerTop",
        WesternPrimitive(WesternPrimitive::CYLINDER,
            /*scale*/ glm::vec3(0.90f, 2.20f, 0.90f),   
            /*color*/ glm::vec3(0.55f, 0.25f, 0.25f))   
    );  
    ...
    auto platform = std::make_unique<WindmillNode>(
        "Platform",
        WesternPrimitive(WesternPrimitive::CUBE,
            /*scale*/ glm::vec3(1.60f, 0.35f, 2.00f),   
            /*color*/ glm::vec3(0.35f, 0.45f, 0.55f))   
    );
    ...
    const glm::vec3 bladeColor(0.80f, 0.72f, 0.58f);    
...
public:
    ...
    { 
        ...
        RESOURCE_ROOT "resources/right.jpg",   
        RESOURCE_ROOT "resources/left.jpg",    
        RESOURCE_ROOT "resources/top.jpg",     
        RESOURCE_ROOT "resources/bottom.jpg",  
        RESOURCE_ROOT "resources/front.jpg",   
        RESOURCE_ROOT "resources/back.jpg"     
        };
    ...
    void shootBullet(int shooterIndex) {
        ...
        if (shooterIndex == 0) {
            start = leftPos + glm::vec3(0.5f, 0.0f, 0.0f); 
            target = rightPos;
        }
        else {
    ...
    void initPathRendering() {
        m_pathLineSegments = m_cameraPath.generateLineSegments(30); 
    ...
    void renderPathCurve() {
        ...
        glm::vec3 pathColor(1.0f, 1.0f, 0.0f); 
...
private:   
    ...
    void handleContinuousKeyboard(float dt)
    {
        ...
        if (m_keyE) m_cameraPosition += m_cameraUp * step; 
        if (m_keyQ) m_cameraPosition -= m_cameraUp * step; 
    }
    ...
    void renderModel(std::vector<GPUMesh>& meshes, const glm::mat4& modelMatrix, bool isGround)
    {
        ...
        for (GPUMesh& mesh : meshes) {
            ...
            glm::vec3 matColor = glm::vec3(0.75f, 0.75f, 0.78f); 
            glUniform3fv(shader.getUniformLocation("materialColor"), 1, glm::value_ptr(matColor));

            if (m_enableEnvironmentMap) {
                int cubemapUnit = MAX_SHADOW_LIGHTS + 2;  
    ...
    void updateCameraViewMode()
    {
        ...
        case ViewMode::Top:
            m_cameraPosition = glm::vec3(0.0f, 10.0f, 0.01f); 
    ...
    void onKeyPressed(int key, int mods)
    {
        ...
        if (key == GLFW_KEY_E) m_keyE = true; 
        if (key == GLFW_KEY_Q) m_keyQ = true;         
...
private:
    ...
    float m_distanceFromTarget{ 5.0f }; 
